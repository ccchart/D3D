...
!!     END DEBUG
 ...
 1234 continue    
   ...
   subroutine update_ghosts(itype, number_of_unknowns_per_point, number_of_points, solution, ierror)
#ifdef HAVE_MPI   
      use mpi
#endif      
      use m_flowgeom
      ...
      integer,           intent(in)    :: number_of_unknowns_per_point         !< == flownode/link/etc.
      integer,           intent(in)    :: number_of_points                     !< == flownodes/links/etc.
      double precision,  intent(inout) :: solution(number_of_unknowns_per_point*number_of_points)
      integer,           intent(out)   :: ierror                               !< error (1) or not (0)
      ...
      if ( itype == ITYPE_S ) then
         ...
         call update_ghost_values(number_of_unknowns_per_point, number_of_points, solution, ITYPE_S, nghostlist_s(ndomains-1), ighostlist_s, &
                                  nghostlist_s, nsendlist_s(ndomains-1), isendlist_s, nsendlist_s, ierror)
      else if ( itype == ITYPE_SALL ) then
         if ( number_of_points /= Ndx) then
            call qnerror('update_ghosts, ITYPE_Sall: numbering error', ' ', ' ')
            goto 1234
         end if
         call update_ghost_values(number_of_unknowns_per_point, number_of_points, solution, ITYPE_Sall, nghostlist_sall(ndomains-1), &
                                  ighostlist_sall, nghostlist_sall, nsendlist_sall(ndomains-1), isendlist_sall, nsendlist_sall, ierror)
         ...
         call update_ghost_values(number_of_unknowns_per_point, number_of_points, solution, ITYPE_U, nghostlist_u(ndomains-1), &
                                  ighostlist_u, nghostlist_u, nsendlist_u(ndomains-1), isendlist_u, nsendlist_u, ierror)
!
!     3D-extension         
         ...
         call update_ghost_values(number_of_unknowns_per_point, number_of_points, solution, ITYPE_S, nghostlist_s(ndomains-1), &
                                  ighostlist_s, nghostlist_s, nsendlist_s(ndomains-1), isendlist_s, nsendlist_s, ierror, nghostlist_s_3D, nsendlist_s_3D, kmxn, kbot)
      else if ( itype == ITYPE_SALL3D ) then
         if ( number_of_points /= Ndkx) then
            call qnerror('update_ghosts, ITYPE_Sall3D: numbering error', ' ', ' ')
            goto 1234
         end if
         call update_ghost_values(number_of_unknowns_per_point, number_of_points, solution, ITYPE_Sall, nghostlist_sall(ndomains-1), ighostlist_sall, nghostlist_sall,& 
                                 nsendlist_sall(ndomains-1), isendlist_sall, nsendlist_sall, ierror, nghostlist_sall_3D, nsendlist_sall_3D, kmxn, kbot)
      else if ( itype == ITYPE_U3D ) then
         if ( number_of_points /= Lnkx) then
            call qnerror('update_ghosts, ITYPE_U3D: numbering error', ' ', ' ')
            goto 1234
         end if
         call update_ghost_values(number_of_unknowns_per_point, number_of_points, solution, ITYPE_U, nghostlist_u(ndomains-1), ighostlist_u, nghostlist_u, &
                                  nsendlist_u(ndomains-1), isendlist_u, nsendlist_u, ierror, nghostlist_u_3D, nsendlist_u_3D, kmxL, Lbot)
         ...
         call update_ghost_values(number_of_unknowns_per_point, number_of_points, solution, ITYPE_U, nghostlist_u(ndomains-1), ighostlist_u, nghostlist_u, &
                                  nsendlist_u(ndomains-1), isendlist_u, nsendlist_u, ierror, nghostlist_u_3Dw, nsendlist_u_3Dw, kmxL+1, Lbot-1)
...
!     overlap
         ...
         if ( jaoverlap == 1 ) then
            call update_ghost_values(number_of_unknowns_per_point, number_of_points, solution, ITYPE_Snonoverlap, &
                                     ...
                                     nsendlist_snonoverlap(ndomains-1), isendlist_snonoverlap, nsendlist_snonoverlap, ierror)
...
!>   3D extension: we assume that kbot/Lbot and kmxn/kmxL match their counterparts in the other domain(s)
   subroutine update_ghost_values(number_of_unknowns_per_point, number_of_points, solution, itype, &
       ...
       error, nghost3d, nsend3d, kmxnL, kbot)
#ifdef HAVE_MPI   
      use mpi
#endif      
      use m_flowgeom
...
!     send data to other procs 
      ...
      do other_rank  = 0, ndomains - 1
         if ( cumulative_numbers_sends(other_rank) >  cumulative_numbers_sends(other_rank - 1) ) then
             ...
             if ( ja3d == 0 ) then
               call create_2d_data_type(other_rank, number_of_unknowns_per_point, ndomains, cumulative_numbers_sends, number_of_send_data, send_list, &
                   size_displacements, displacements, send_type(nr_mpi_sends), send_counts(nr_mpi_sends), error)
            else
               call create_3d_data_type(other_rank, number_of_unknowns_per_point, ndomains, cumulative_numbers_sends, number_of_send_data, send_list, &
                   number_of_points, kbot, kmxnL, size_displacements, displacements, blocks, send_type(nr_mpi_sends), send_counts(nr_mpi_sends), error)
           end if

           call mpi_type_commit(send_type(nr_mpi_sends), error)
           tag  = itype
           call mpi_isend(solution, 1, send_type(nr_mpi_sends), other_rank, tag, DFM_COMM_DFMWORLD, send_requests(nr_mpi_sends), error)
        end if
     end do
...
!    receive data from other procs 
     ...
     do other_rank  = 0, ndomains - 1
        if ( cumulative_numbers_ghosts(other_rank) >  cumulative_numbers_ghosts(other_rank - 1) ) then
            ...
            if ( ja3d == 0 ) then
                call create_2d_data_type(other_rank, number_of_unknowns_per_point, ndomains, cumulative_numbers_ghosts, number_of_ghosts, ghost_list, &
                   size_displacements, displacements, recv_type(nr_mpi_recvs), recv_counts(nr_mpi_recvs), error)
            else
                call create_3d_data_type(other_rank, number_of_unknowns_per_point, ndomains, cumulative_numbers_ghosts, number_of_ghosts, ghost_list, &
                   number_of_points, kbot, kmxnL, size_displacements, displacements, blocks, recv_type(nr_mpi_recvs), recv_counts(nr_mpi_recvs), error)
            end if
            
           call mpi_type_commit(recv_type(nr_mpi_recvs), error)
           tag  = itype
           call mpi_irecv(solution, 1, recv_type(nr_mpi_recvs), other_rank, tag, DFM_COMM_DFMWORLD, recv_requests(nr_mpi_recvs), error)
        end if
    end do
