...
subroutine update_mergebuffer(mergehandle, buffer_size, mergebuffer, jampi, my_rank, ndomains, DFM_COMM_DFMWORLD)
   use m_alloc
   use precision
   use m_mormerge
#ifdef HAVE_MPI
   ...
   integer                           :: buffers_sizes(ndomains)            !< number of points (ndxi*lsedtot) over domains/procs
   ...
   if ( jampi == 0 ) then
      ...
      call mpi_gather(buffer_size, 1, mpi_integer, buffers_sizes, 1, mpi_integer, 0, DFM_COMM_DFMWORLD, error)
      if ( my_rank == 0 ) then
